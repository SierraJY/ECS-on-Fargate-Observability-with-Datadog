...
- Phase 5: ECS 컨트롤 플레인(배포·오케스트레이션 내부 동작) 관측 — 이건 반대로 3장 제약의 연장선(계획 단계)
...
- ECS Action Logs
    - AWS가 2026-07 발표한 기능. ECS가 배포·managed daemon 갱신 중 내부적으로 수행한 작업을 구조화 로그로 노출
...
### 3.3 ECS 컨트롤 플레인 자체의 불투명성 (Phase 5에서 해결 예정)

- 3.1/3.2가 각각 "호스트에 접근할 수단이 없다", "Envoy 내부에 접근할 수단이 없다"는 제약이라면, 이건 한 단계 더 위 계층의 같은 성격 문제다 — ECS가 배포·서비스 업데이트·managed daemon 갱신 과정에서 **사용자를 대신해 내부적으로 수행하는 판단과 작업 자체**가 불투명하다
- 정확히는 Fargate 고유 제약이 아니라 **ECS 자체의 제약**이다(EC2 launch type ECS에도 동일하게 적용됨) — 그럼에도 3장에 포함시키는 이유는, "AWS 관리형 서비스가 특정 레이어를 완전히 추상화하면서 생기는 구조적 공백"이라는 3.1/3.2와 동일한 패턴이기 때문이다(4.6/4.7의 RUM·DBM처럼 제약과 무관하게 관측 범위 자체를 넓히는 것과는 다름)
...
### 4.5 ECS 컨트롤 플레인 관측 (Action Logs, Phase 5에서 구현 예정)

- 3.3에서 제기한 제약(ECS가 배포·오케스트레이션 중 내부적으로 수행하는 판단·작업이 불투명한 문제)에 대한 self-service 해결책 — 4.1~4.4와 같은 층위의 "제약 → 해결" 축이다(4.6/4.7의 RUM·DBM처럼 제약과 무관한 범위 확장이 아님)
- 2026-07 AWS가 발표한 [Amazon ECS Action Logs](https://aws.amazon.com/about-aws/whats-new/2026/07/amazon-ecs-action-logs/) 기능을 채택 — ECS가 배포/managed daemon 갱신 중 수행한 내부 작업을 이벤트명·로그 레벨(INFO/WARN/ERROR)·리소스 ARN·상태 사유가 포함된 구조화 JSON으로 노출
- **채택 파이프라인(계획)**: Action Logs 활성화(클러스터 단위) → Kinesis Data Firehose(직접 전송) → Datadog — 4.2.1(VPC Flow Logs)과 동일한 패턴 재사용, Firehose를 직접 목적지로 선택할 수 있어 CloudWatch Logs 구독 필터 같은 중간 단계 불필요
- 진행 상세는 `PHASE5_작업지시서.md` 참고(작성 시점 기준 계획 단계, 아직 미구현)

### 4.6 프론트엔드 관측 (RUM, Phase 3에서 구현)

- 3장의 제약과는 무관한 별도 축 — 4.1~4.5는 전부 백엔드/인프라 관측이었고, 사용자가 실제로 겪는 브라우저 단(로딩 시간, 클릭, JS 에러)은 관측 공백으로 남아 있었다는 점에서 출발한 범위 확장이다
...
### 4.7 DB 레벨 관측 (DBM, Phase 3에서 구현)

- 이 역시 3장의 제약과는 무관한 범위 확장 — 4.4(APM)가 서비스 간 호출 흐름은 보여주지만, 그 안에서 실행되는 개별 SQL 쿼리의 성능(실행 횟수, 평균 시간, 락 대기 등)은 여전히 블랙박스였다는 공백에서 출발한다
