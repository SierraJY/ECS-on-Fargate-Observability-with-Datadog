...
- 메트릭
    - 표준적인 Envoy 관측 방법(admin/stats 엔드포인트 노출)이 원천적으로 불가능하다
    - "기본값이 꺼져 있어서 켜야 하는" 문제가 아니라 "사용자가 켤 수 있는 수단 자체가 없는" 문제다
    - → AWS가 큐레이션해서 외부로 노출하는 대체 지점(CloudWatch 네이티브 메트릭)으로 4.2.2에서 해결
- 로그
    - 3.1과 마찬가지로 호스트의 로그 파일(`/var/log/...`)에 접근하거나 호스트 단위로 로그 수집 에이전트를 설치하는 방식이 불가능
    - 심지어 Envoy 컨테이너는 Task Definition에서 조차 접근 불가
