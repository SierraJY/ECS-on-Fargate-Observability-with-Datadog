...
- `Inventory`는 앱 기동 시 `seats` 테이블이 없으면 자동 생성
    - **정정(2026-08-03, Phase 4)**: 비행기(단일 행 `A1`~`A20`, 20석) → 영화관 좌석도(행 A~F, 3구역, 사다리꼴, 150석)로 전환. 시드 방식도 "비어있을 때만"에서 **멱등 리컨실 방식**으로 변경 — 매 기동 시 목표 seat_id 집합을 `INSERT ... ON CONFLICT DO NOTHING`으로 채우고, 목표 집합에 없는 옛 seat_id는 `DELETE`로 정리. 재배포만 하면 항상 최신 좌석 구성으로 수렴함
    - 행별 구성(앞줄일수록 좁고 뒷줄일수록 넓은 사다리꼴): A행 20석(6~9, 10~21, 22~25), B행 22석, C행 24석, D행 26석, E행 28석, F행 30석(1~9, 10~21, 22~30) — 중앙 구역(10~21, 12석)은 모든 행 공통, 좌/우 구역만 행마다 확장
